...
int IRrecv::getIRRemoteCode()
{
    ...
    int result = 0; //##Se determinó que devolver 0 en vez de -1 cuando no recibe nada coherente, es mejor
                    ...
                    // 
                    // It was discovered that return 0 instead of -1 when it receives nothing coherent,
                    // it is better for certain activities, such as the music with the remote control
                    // (especially when working with kids). This is why below 10 is returned when it detects a 0.
    ...
    if (decode(&results))
    {
        ...
        if (results.decode_type == RC5)  //By now, only supports RC5 Format.
        {
            ...
            if ( (strcmp(command, "0") == 0) || (strcmp(command, "800") == 0)) //##Ver si el cero es correcto. See if the zero is correct
